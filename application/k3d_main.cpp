...
{
	// Logging options
	k3d::bool_t show_timestamps = false;
	k3d::bool_t show_process = false;
	k3d::bool_t syslog = false;
	k3d::bool_t color_level = false;
	k3d::log_level_t minimum_log_level = k3d::K3D_LOG_LEVEL_WARNING;

	// Default paths
	k3d::filesystem::path default_ngui_path;
	k3d::filesystem::path default_nui_path;
	k3d::filesystem::path default_options_path;
	k3d::filesystem::path default_pyui_path;
	k3d::filesystem::path default_qtui_path;
	k3d::filesystem::path default_shader_cache_path;
	k3d::filesystem::path default_share_path;
	k3d::string_t default_plugin_paths;

	// Overridden paths
	k3d::filesystem::path override_locale_path;
	k3d::filesystem::path options_path;
	k3d::filesystem::path shader_cache_path;
	k3d::filesystem::path share_path;
	k3d::filesystem::path user_interface_path;
	k3d::string_t plugin_paths;

	// Runtime state
	std::unique_ptr<k3d::ievent_loop> user_interface;
};

K3DAppState g_app_state;
...
void startup_message_handler(const k3d::string_t& Message)
{
	...
	if(g_app_state.user_interface)
		g_app_state.user_interface->startup_message_handler(Message);
}
...
{
	if(g_app_state.user_interface)
		g_app_state.user_interface->stop_event_loop();
...
void set_default_options(k3d::bool_t& Quit, k3d::bool_t& Error)
{
	...
	const auto executable_dir = k3d::system::executable_path().branch_path();
	const auto user_dir = k3d::system::get_home_directory() / k3d::filesystem::generic_path(".k3d");

	g_app_state.default_ngui_path = executable_dir / k3d::filesystem::generic_path("../" K3D_LIBDIR "/k3d/plugins/k3d-ngui.module");
	g_app_state.default_nui_path = executable_dir / k3d::filesystem::generic_path("../" K3D_LIBDIR "/k3d/plugins/k3d-nui.module");
	g_app_state.default_options_path = user_dir / k3d::filesystem::generic_path("options.k3d");
	g_app_state.default_plugin_paths = (executable_dir / k3d::filesystem::generic_path("../" K3D_LIBDIR "/k3d/plugins")).native_filesystem_string();
	g_app_state.default_pyui_path = executable_dir / k3d::filesystem::generic_path("../" K3D_LIBDIR "/k3d/plugins/k3d-pyui.module");
	g_app_state.default_qtui_path = executable_dir / k3d::filesystem::generic_path("../" K3D_LIBDIR "/k3d/plugins/k3d-qtui.module");
	g_app_state.default_shader_cache_path = user_dir / k3d::filesystem::generic_path("shadercache");
	g_app_state.default_share_path = executable_dir / k3d::filesystem::generic_path("../share/k3d");

#ifdef K3D_ENABLE_OSX_BUNDLE
	g_app_state.user_interface_path = g_app_state.default_qtui_path;
#else
	g_app_state.user_interface_path = g_app_state.default_ngui_path;
#endif

	// Setup path options based on the defaults ...
	g_app_state.options_path = g_app_state.default_options_path;
	g_app_state.plugin_paths = g_app_state.default_plugin_paths;
	g_app_state.shader_cache_path = g_app_state.default_shader_cache_path;
	g_app_state.share_path = g_app_state.default_share_path;

	// Optionally override paths using environment variables ...
	if(!k3d::system::getenv("K3D_LOCALE_PATH").empty())
		g_app_state.override_locale_path = k3d::filesystem::native_path(k3d::ustring::from_utf8(k3d::system::getenv("K3D_LOCALE_PATH")));

	if(!k3d::system::getenv("K3D_OPTIONS_PATH").empty())
		g_app_state.options_path = k3d::filesystem::native_path(k3d::ustring::from_utf8(k3d::system::getenv("K3D_OPTIONS_PATH")));

	if(!k3d::system::getenv("K3D_PLUGIN_PATHS").empty())
		g_app_state.plugin_paths = k3d::system::getenv("K3D_PLUGIN_PATHS");

	if(!k3d::system::getenv("K3D_SHADER_CACHE_PATH").empty())
		g_app_state.shader_cache_path = k3d::filesystem::native_path(k3d::ustring::from_utf8(k3d::system::getenv("K3D_SHADER_CACHE_PATH")));

	if(!k3d::system::getenv("K3D_SHARE_PATH").empty())
		g_app_state.share_path = k3d::filesystem::native_path(k3d::ustring::from_utf8(k3d::system::getenv("K3D_SHARE_PATH")));

	if(!k3d::system::getenv("K3D_USER_INTERFACE_PATH").empty())
		g_app_state.user_interface_path = k3d::filesystem::native_path(k3d::ustring::from_utf8(k3d::system::getenv("K3D_USER_INTERFACE_PATH")));
...
const arguments_t parse_log_arguments(const arguments_t& Arguments, k3d::bool_t& Quit, k3d::bool_t& Error)
{
	...
	for(const auto& argument : Arguments)
	{
		...
		{
			g_app_state.show_timestamps = true;
		}
		else if(argument.string_key == "show-process")
		{
			g_app_state.show_process = true;
		}
		else if(argument.string_key == "syslog")
		{
			g_app_state.syslog = true;
		}
		else if(argument.string_key == "color")
		{
			g_app_state.color_level = true;
		}
		else if(argument.string_key == "no-color")
		{
			g_app_state.color_level = false;
		}
		else if(argument.string_key == "log-level")
		{
			if(argument.value[0] == "warning")
				g_app_state.minimum_log_level = k3d::K3D_LOG_LEVEL_WARNING;
			else if(argument.value[0] == "information")
				g_app_state.minimum_log_level = k3d::K3D_LOG_LEVEL_INFO;
			else if(argument.value[0] == "debug")
				g_app_state.minimum_log_level = k3d::K3D_LOG_LEVEL_DEBUG;
			else
			{
	...
	k3d::log_show_timestamps(g_app_state.show_timestamps);
	k3d::log_set_tag(g_app_state.show_process ? "[k3d]" : k3d::string_t());
	k3d::log_color_level(g_app_state.color_level);
	k3d::log_show_level(true);
	k3d::log_syslog(g_app_state.syslog);
	k3d::log_minimum_level(g_app_state.minimum_log_level);
	...
	for(const auto& argument : Arguments)
	{
		...
		else if(argument.string_key == "ui")
		{
			if(argument.value[0] == "nui" || argument.value[0] == "none")
				g_app_state.user_interface_path = g_app_state.default_nui_path;
			else if(argument.value[0] == "ngui")
				g_app_state.user_interface_path = g_app_state.default_ngui_path;
			else if(argument.value[0] == "qtui")
				g_app_state.user_interface_path = g_app_state.default_qtui_path;
			else if(argument.value[0] == "pyui")
				g_app_state.user_interface_path = g_app_state.default_pyui_path;
			else
				g_app_state.user_interface_path = k3d::filesystem::native_path(k3d::ustring::from_utf8(argument.value[0]));
		}
		else if(argument.string_key == "plugins")
		{
			g_app_state.plugin_paths = argument.value[0];
			g_app_state.plugin_paths = k3d::replace_all("&", g_app_state.default_plugin_paths, g_app_state.plugin_paths);
		}
		else if(argument.string_key == "shadercache")
		{
			g_app_state.shader_cache_path = k3d::filesystem::native_path(k3d::ustring::from_utf8(argument.value[0]));
		}
		else if(argument.string_key == "share")
		{
			g_app_state.share_path = k3d::filesystem::native_path(k3d::ustring::from_utf8(argument.value[0]));
		}
		else if(argument.string_key == "options")
		{
			g_app_state.options_path = k3d::filesystem::native_path(k3d::ustring::from_utf8(argument.value[0]));
		}
		...
		{
			g_app_state.override_locale_path = k3d::filesystem::native_path(k3d::ustring::from_utf8(argument.value[0]));
		}
		else
		{
			unused.push_back(argument);
...
void check_dependencies(k3d::bool_t& Quit, k3d::bool_t& Error)
{
	...
	k3d::log() << info << "options file: " << g_app_state.options_path.native_console_string() << std::endl;
	k3d::log() << info << "plugin path(s): " << g_app_state.plugin_paths << std::endl;
	k3d::log() << info << "shader cache path: " << g_app_state.shader_cache_path.native_console_string() << std::endl;
	k3d::log() << info << "share path: " << g_app_state.share_path.native_console_string() << std::endl;
	k3d::log() << info << "user interface: " << g_app_state.user_interface_path.native_console_string() << std::endl;
	...
	if(g_app_state.options_path.empty())
	{
		handle_error("Options path must be specified using --options <path>.", Quit, Error);
		return;
	}

	k3d::filesystem::create_directories(g_app_state.options_path.branch_path());
	if(!k3d::filesystem::exists(g_app_state.options_path.branch_path()))
	{
		handle_error("Options directory [" + g_app_state.options_path.branch_path().native_console_string() + "] does not exist and could not be created.", Quit, Error);
		return;
	}

	// Every specified plugin path must exist ...
	const auto plugin_paths = k3d::system::decompose_path_list(g_app_state.plugin_paths);
	...
	if(g_app_state.shader_cache_path.empty())
	{
		handle_error("Shader cache path must be specified using --shadercache <path>.", Quit, Error);
		return;
	}

	k3d::filesystem::create_directories(g_app_state.shader_cache_path);
	if(!k3d::filesystem::exists(g_app_state.shader_cache_path))
	{
		handle_error("Shader cache path [" + g_app_state.shader_cache_path.native_console_string() + "] does not exist and could not be created.", Quit, Error);
		return;
	}

	// The share path must exist ...
	if(!k3d::filesystem::exists(g_app_state.share_path))
	{
		handle_error("Share path [" + g_app_state.share_path.native_console_string() + "] does not exist.", Quit, Error);
		return;
	}
}
...
{
	const auto& ui_path = g_app_state.user_interface_path;
	const auto module_name = ui_path.native_console_string();
	if(!ui_path.empty() && !k3d::filesystem::exists(ui_path))
	{
		handle_error("UI plugin module [" + module_name + "] does not exist", Quit, Error);
		return;
	}
	Plugins.load_module(ui_path, k3d::plugin_factory_collection::IGNORE_PROXIES);
	...
	k3d::ievent_loop* ui_ptr = k3d::plugin::create<k3d::ievent_loop>(**Plugins.factories().begin());
	if(!ui_ptr)
	{
	...
	if(!dynamic_cast<k3d::iuser_interface*>(ui_ptr))
	{
		delete ui_ptr;
	...
	g_app_state.user_interface.reset(ui_ptr);
}
...
{
	Plugins.load_modules(g_app_state.plugin_paths, true, k3d::plugin_factory_collection::LOAD_PROXIES);
}
...
const arguments_t parse_runtime_arguments(const arguments_t& Arguments, k3d::bool_t& Quit, k3d::bool_t& Error, k3d::iapplication& Application)
{
	...
	for(const auto& argument : Arguments)
	{
		if(argument.string_key == "script" || argument.string_key == "script-file")
		{
			...
			else if(argument.string_key == "script-file")
			{
				...
				else
				{
					...
					const auto script_path = k3d::filesystem::native_path(k3d::ustring::from_utf8(script_name));
		...
		{
			const auto extensions = k3d::gl::extension::list();
...
{
	const auto factories = k3d::plugin::factory::lookup();
	for(auto factory : factories)
	{
		...
		if(auto* scripted_action = dynamic_cast<k3d::iscripted_action*>(plugin))
		{
			k3d::iscript_engine::context context;
...
int k3d_main(std::vector<k3d::string_t> raw_arguments)
{
	...
	try
	{
		...
		k3d::set_shader_cache_path(g_app_state.shader_cache_path);

		// Set the share path ...
		k3d::set_share_path(g_app_state.share_path);

#ifdef K3D_ENABLE_NLS
		k3d::filesystem::path locale_path = g_app_state.share_path / k3d::filesystem::generic_path("locale");
		if(!g_app_state.override_locale_path.empty())
			locale_path = g_app_state.override_locale_path;
		...
		// Load user options ...
		k3d::options::file_storage user_options(g_app_state.options_path);
		...
		if(quit) return error ? 1 : 0;
		return_val_if_fail(g_app_state.user_interface, 1);
		...
		for(const auto& argument : arguments)
		{
			...
			{
				boost::program_options::options_description ui_description("User interface options");
				g_app_state.user_interface->get_command_line_arguments(ui_description);
				std::cout << ui_description << "\n";
				return 0;
			}
		}

		// Register our user interface ...
		k3d::set_user_interface(*dynamic_cast<k3d::iuser_interface*>(g_app_state.user_interface.get()));

		// Give the UI a chance to handle command-line arguments ...
		arguments = g_app_state.user_interface->parse_startup_arguments(arguments, quit, error);
		if(quit) return error ? 1 : 0;
		...
		// Setup a render farm ...
		k3d::network_render_farm render_farm(g_app_state.options_path);
		...
		g_app_state.user_interface->display_user_interface();
		...
		arguments = g_app_state.user_interface->parse_runtime_arguments(arguments, quit, error);
		if(quit) return error ? 1 : 0;
		...
		// Main UI event-loop ...
		g_app_state.user_interface->start_event_loop();
